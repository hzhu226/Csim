...
void access_data(mem_addr_t addr) {
  mem_addr_t t_bit = addr >> (b + s);
  mem_addr_t s_bit = addr >> b;
  s_bit = s_bit & ((int)pow(2,s) - 1);
  t_bit = t_bit & ((int)pow(2, 64 - (b+s)) -1); 
  
  int hit = 0;
  int add = 0;
  for (int i = 0; i < E; i++) {
    
    if ((cache[s_bit][i].valid == 1) && cache[s_bit][i].tag == t_bit) {
      hit = 1;
      max++;
      hit_cnt++;
      cache[s_bit][i].counter = max;
      if (verbosity)
        printf("hit ");
      break;
    }
    if (cache[s_bit][i].valid == 0) {
      ...
      max++;
      miss_cnt++;
      cache[s_bit][i].tag = t_bit;
      cache[s_bit][i].valid = 1;
      cache[s_bit][i].counter = max;
      if (verbosity)
        printf("miss ");
      break;
  ...
  if (add == 0 && hit ==0) {
    ...
    int curr = 0;
    for (int i = 0; i < E; i++) {
      if (cache[s_bit][i].counter < min) {
        min = cache[s_bit][i].counter;
        curr = i;
      }
    }
    cache[s_bit][curr].tag = t_bit;
    
    cache[s_bit][curr].counter = max;
  }
